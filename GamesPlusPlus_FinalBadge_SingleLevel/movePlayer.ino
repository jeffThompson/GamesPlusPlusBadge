...
// NOTE: dying has been disabled - walking offscreen now works like hitting a wall (nothing happens)

void movePlayer(char dir) {

  // north
  if (dir == 'n') {
    if (level[y-1][x] == 4) {            // fall through to next level below
      y--;                               // move to hole
      rumble(4);                         // play falling rumble
      setPosition();                     // reset x/y coords
